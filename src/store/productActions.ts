...
export const getFavouriteProducts = (): ThunkAction<void, RootState, unknown, AnyAction> => {
    return async (dispatch, getState) => {

            const response: ProductModel[] =JSON.parse(localStorage.getItem("favourites")!) 
            dispatch(productActions.setFavouriteProducts(response))
       

    }
}

export const deleteProductToFavouriteList=(product_id:string):ThunkAction<void,RootState,unknown,AnyAction>=>{
    return async(dispatch,getState)=>{
        const response=getState().product.favouriteProducts.filter(({_id})=>_id!==product_id)
        localStorage.removeItem("favourites")
        localStorage.setItem("favourites",JSON.stringify(response))
        dispatch(productActions.deleteFavouriteProduct(response));
    } 
}